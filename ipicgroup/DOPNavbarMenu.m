...
+ (DOPNavbarMenuItem *)ItemWithTitle:(NSString *)title {
...
- (instancetype)initWithItems:(NSArray *)items width:(CGFloat)width maximumNumberInRow:(NSInteger)max color:(NSMutableArray*)menu_color{
    ...
    _separatarColor = [UIColor colorWithRed:0.129 green:0.569 blue:0.208 alpha:1];
...
- (void)layoutSubviews {
    ...
    [self.items enumerateObjectsUsingBlock:^(DOPNavbarMenuItem *obj, NSUInteger idx, BOOL *stop) {
        ...
        button.tag = idx;
        [button setBackgroundColor:_itemcolor[counter++]];
        ...
        UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(0, buttonHeight/2, buttonWidth, 32)];
        label.text = obj.title;
        //NSLog(@"%@",_itemcolor);
        //[label setBackgroundColor:_itemcolor[counter++]];
